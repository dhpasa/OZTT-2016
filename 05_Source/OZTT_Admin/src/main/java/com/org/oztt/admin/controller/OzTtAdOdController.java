...
public class OzTtAdOdController extends BaseController {
	...
	public String init(Model model, HttpServletRequest request,
			HttpSession session, String orderNo, String pageNo) {
		try {
			OzTtAdOdDto ozTtAdOdDto = orderService
					.getOrderDetailForAdmin(orderNo);
			if (CommonEnum.DeliveryMethod.PICK_INSTORE.getCode().equals(
					ozTtAdOdDto.getDeliveryMethodFlag())) {
				// 来店自提
				TCustomerBasicInfo baseInfo = customerService
						.selectBaseInfoByCustomerNo(ozTtAdOdDto.getCustomerNo());
				TCustomerSecurityInfo securityInfo = customerService
						.getCustomerSecurityByCustomerNo(ozTtAdOdDto
								.getCustomerNo());
				ozTtAdOdDto.setReceiver(baseInfo.getNickname());
				ozTtAdOdDto.setPhone(securityInfo.getTelno());
			} else if (CommonEnum.DeliveryMethod.HOME_DELIVERY.getCode()
					.equals(ozTtAdOdDto.getDeliveryMethodFlag())) {
				// 送货上门
			}
			model.addAttribute("ozTtAdOdDto", ozTtAdOdDto);
			model.addAttribute("OrderAmountAndFre",
					new BigDecimal(ozTtAdOdDto.getOrderAmount())
							.add(new BigDecimal(
									ozTtAdOdDto.getYunfei() == null ? "0"
											: ozTtAdOdDto.getYunfei())));
	...
	public String changeStatus(Model model, HttpServletRequest request,
			HttpSession session, String orderNo, String status, String pageNo) {
		try {
			...
			if (CommonEnum.HandleFlag.NOT_PAY.getCode().equals(status)) {
				...
				orderService.updateRecordAfterPay(orderNo, "ADMIN", session,
						CommonConstants.TRANSACTION_SERIAL_NO_MOCK);
			} else {
				// 检索当前订单，更新状态为已经付款
			...
			return "redirect:/OZ_TT_AD_OD/init?orderNo=" + orderNo + "&pageNo="
					+ pageNo;
		} catch (Exception e) {
			logger.error(e.getMessage());
			return CommonConstants.ERROR_PAGE;
